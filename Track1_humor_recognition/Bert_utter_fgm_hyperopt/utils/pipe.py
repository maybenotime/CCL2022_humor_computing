...
def _prepare_data_bundle(tokenizer, data_bundle):
    ...
    for name in data_bundle.get_dataset_names():    #重构各个数据集，field主要有input和target
        ...
        for ins in ds:
            ...
            token_type_ids = tokenizer.create_token_type_ids_from_sequences(sen_with_speaker,context)
            ins['token_type_ids'] = token_type_ids
            input = [cls_id] + sen_with_speaker + [sep_id] + context + [sep_id]  #构造input
            input = input[:400]                                    #截断，最大输入长度是400
            ins['input'] = input
            new_ds.append(ins)
    ...
    data_bundle.set_input('input','token_type_ids')              #将该field设置为input, 对data_bundle中所有的dataset执行
